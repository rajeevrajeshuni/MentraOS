...
export class LogoutUtils {
  ...
  public static async performCompleteLogout(): Promise<void> {
    ...
    try {
      ...
      // Step 2: Clear backend communication tokens
      await this.clearBackendTokens();

      // Step 3: Stop and cleanup core services
      await this.stopCoreServices();

      // Step 4: Clear all app settings and user data
      await this.clearAppSettings();

      // Step 5: Clear any remaining auth-related storage
      await this.clearAuthStorage();

      // Step 6: Reset status providers and event emitters
  ...
  private static async clearAppSettings(): Promise<void> {
    ...
    try {
      ...
      const settingsToClear = Object.values(SETTINGS_KEYS).filter(
        key => !settingsToKeep.includes(key)
      );
