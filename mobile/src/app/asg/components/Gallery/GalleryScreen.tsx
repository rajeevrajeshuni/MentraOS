...
export function GalleryScreen({deviceModel = "ASG Glasses"}: GalleryScreenProps) {
  ...
  const loadDownloadedPhotos = useCallback(async () => {
    try {
      ...
      const photoInfos = Object.values(downloadedFiles).map(file => 
        localStorageService.convertToPhotoInfo(file)
      )
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (downloadResult.downloaded.length > 0) {
        const deleteResult = await asgCameraApi.deleteFilesFromServer(
  ...
  const handleDeleteDownloadedPhoto = async (photo: PhotoInfo) => {
    Alert.alert(
      ...
      `Are you sure you want to delete "${photo.name}" from local storage?`,
      [
        {text: "Cancel", style: "cancel"},
        {
          text: "Delete",
    ...
    >
        ...
        {isLoading ? (
          ...
          <View style={themed($emptyContainer)}>
            <Text style={themed($emptyText)}>
              {isServerTab ? "No photos on server" : "No downloaded photos"}
            </Text>
          </View>
        ) : (
      ...
      >
        <View style={themed($modalOverlay)}>
          ...
          >
            {selectedPhoto && (
              <Image source={{uri: selectedPhoto.url}} style={themed($modalImage)} />
            )}
          </TouchableOpacity>
        </View>
      </Modal>
