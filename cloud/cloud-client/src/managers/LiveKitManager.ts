...
export class LiveKitManager extends EventEmitter {
  ...
  async connectAndPublish(): Promise<void> {
    ...
    if (this.options.publishAudioFilePath) {
      ...
      this.emit('warning', { message: 'No publisher configured. Provide publishAudioFilePath.' });
    }
  }
  ...
  async connectForCustomPublisher(): Promise<void> {
    if (!this.info) throw new Error('LiveKit info not set');
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { Room, AudioSource, LocalAudioTrack, TrackSource } = require('@livekit/rtc-node');
    const room = new Room();
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        console.log(`[LiveKitManager] Connection attempt ${attempt + 1}/${maxRetries}`);
        ...
        lastError = error;
        console.error(`[LiveKitManager] Connection attempt ${attempt + 1} failed:`, error.message);
        
        if (attempt < maxRetries - 1) {
          console.log(`[LiveKitManager] Retrying in ${retryDelays[attempt]}ms...`);
    ...
    const source = new AudioSource({ sampleRate: this.targetRate, numChannels: 1 });
    const track = LocalAudioTrack.createAudioTrack('microphone', { source, sourceType: TrackSource.Microphone });
  ...
  sendPcmChunk(chunk: Buffer, sampleRate: number = 16000): void {
    ...
    const resampled = sampleRate === this.targetRate ? chunk : this.resampleLinear(chunk, sampleRate, this.targetRate);
    ...
    if (!this.pushTimer) {
      this.pushTimer = setInterval(() => {
        try {
          ...
          const audioFrame = { data: samples, sampleRate: this.targetRate, numChannels: 1, samplesPerChannel: samples.length };
          if (typeof (this.customSource as any).captureFrame === 'function') {
            (this.customSource as any).captureFrame(audioFrame);
          } else if (typeof (this.customSource as any).pushFrame === 'function') {
  ...
  private async publishFromFile(room: any, filePath: string): Promise<void> {
    if (this.isBrowser()) {
      this.emit('warning', { message: 'publishFromFile is Node-only. Use useBrowserMic in browser.' });
      return;
    }
    ...
    const fs = require('fs');
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { AudioSource, LocalAudioTrack, TrackSource } = require('@livekit/rtc-node');
    ...
    const resampled = sampleRate === targetRate ? monoPcm : this.resampleLinear(monoPcm, sampleRate, targetRate);
    ...
    const pushNext = async () => {
      ...
      const audioFrame = { data: samples, sampleRate: targetRate, numChannels: 1, samplesPerChannel: samples.length };
      if (typeof source.captureFrame === 'function') source.captureFrame(audioFrame);
      else if (typeof (source as any).pushFrame === 'function') (source as any).pushFrame(audioFrame);
      setTimeout(pushNext, 10);
    };
    pushNext();
  }

  private parseWav(wavData: Buffer): { sampleRate: number; channels: number; pcm: Buffer } | null {
  ...
  private resampleLinear(pcm: Buffer, fromRate: number, toRate: number): Buffer {
  ...
  private async publishFromBrowserMic(_room: any): Promise<void> { /* no-op in Node */ this.emit('warning', { message: 'Browser mic not supported in Node' }); }

  private isBrowser(): boolean {
    return typeof window !== 'undefined' && typeof (window as any).navigator !== 'undefined';
  }
}
