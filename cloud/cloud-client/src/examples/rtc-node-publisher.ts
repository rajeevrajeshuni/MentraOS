...
async function mintPublishToken(serverHttp: string, identity: string, roomName: string) {
...
async function main() {
  ...
  const { url, token } = await mintPublishToken(serverHttp, `rtc-node-pub:${email}`, email);
  ...
  const track = LocalAudioTrack.createAudioTrack('rtc-node-tone', { source, sourceType: TrackSource.Microphone });
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    for (let i = 0; i < samplesPer10ms; i++) {
      ...
      const sample = Math.sin((2 * Math.PI * 440 * t) / targetRate) * 0.5 * 32767;
      frame[i] = Math.floor(sample);
    }
    const audioFrame = { data: frame, sampleRate: targetRate, numChannels: 1, samplesPerChannel: frame.length };
    if (typeof source.captureFrame === 'function') source.captureFrame(audioFrame);
    else if (typeof source.pushFrame === 'function') (source as any).pushFrame(audioFrame);
    await delay(10);
  }
