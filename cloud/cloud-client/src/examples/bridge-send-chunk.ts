...
async function main() {
  ...
  const userId = process.env.TEST_EMAIL ? `diagnostic-publisher:${process.env.TEST_EMAIL}` : 'diagnostic-publisher:user@example.com';
  ...
  const res = await fetch(`${server}/api/livekit/token`, {
    ...
    body: JSON.stringify({ identity: userId, roomName: userId, mode: 'publish' })
  });
