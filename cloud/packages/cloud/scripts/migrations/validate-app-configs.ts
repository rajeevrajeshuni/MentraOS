...
function formatAppResult(result: AppValidationResult): string {
  ...
  if (DETAILED_OUTPUT || result.issues.length > 0) {
    lines.push(`   ✓ Basic info: ${result.hasName ? '✓' : '✗'} name, ${result.hasDescription ? '✓' : '✗'} description`);
    lines.push(`   ✓ URLs: ${result.hasPublicUrl ? '✓' : '✗'} public, ${result.hasLogoUrl ? '✓' : '✗'} logo, ${result.hasWebviewUrl ? '✓' : '✗'} webview`);
    lines.push(`   ✓ Config: ${result.settingsCount} settings, ${result.toolsCount} tools, ${result.permissionsCount} permissions`);
...
async function validate() {
  try {
    ...
    console.log(`   • Settings: ${stats.appsWithSettings}/${stats.totalApps} (${Math.round(stats.appsWithSettings/stats.totalApps*100)}%)`);
    console.log(`   • Tools: ${stats.appsWithTools}/${stats.totalApps} (${Math.round(stats.appsWithTools/stats.totalApps*100)}%)`);
    console.log(`   • Permissions: ${stats.appsWithPermissions}/${stats.totalApps} (${Math.round(stats.appsWithPermissions/stats.totalApps*100)}%)`);
    console.log('');
    console.log('📝 Apps with basic info:');
    console.log(`   • Name: ${stats.appsWithName}/${stats.totalApps} (${Math.round(stats.appsWithName/stats.totalApps*100)}%)`);
    console.log(`   • Description: ${stats.appsWithDescription}/${stats.totalApps} (${Math.round(stats.appsWithDescription/stats.totalApps*100)}%)`);
    console.log(`   • Public URL: ${stats.appsWithPublicUrl}/${stats.totalApps} (${Math.round(stats.appsWithPublicUrl/stats.totalApps*100)}%)`);
    console.log('');
    ...
    console.log('');
    console.log(`⚠️  Apps with issues: ${stats.appsWithIssues}/${stats.totalApps} (${Math.round(stats.appsWithIssues/stats.totalApps*100)}%)`);
    ...
    if (stats.appsWithSettings < stats.totalApps * 0.5) {
      console.log('📄 Consider running migration with --force to import configs for apps without settings');
    }

    if (stats.appsWithName < stats.totalApps) {
      console.log('📝 Some apps are missing names - these may need manual updates');
    }
