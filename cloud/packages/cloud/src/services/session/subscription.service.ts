...
export class SubscriptionService {
  ...
  hasPCMTranscriptionSubscriptions(sessionId: string): { hasMedia: boolean; hasPCM: boolean; hasTranscription: boolean } {
    let hasMedia = false;
    let hasPCM = false;
    ...
    for (const [key, subs] of this.subscriptions.entries()) {
      ...
      for (const sub of subs) {
        if (sub === StreamType.AUDIO_CHUNK) {
          hasPCM = true;
          hasMedia = true;
        } else if (sub === StreamType.TRANSLATION || sub === StreamType.TRANSCRIPTION) {
