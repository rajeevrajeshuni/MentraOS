...
export class ProviderSelector {
  ...
  async selectProvider(
    ...
    if (!excludeList.includes(defaultProviderType)) {
      ...
      if (defaultProvider?.supportsSubscription(subscription) && 
        ...
        this.logger.warn({
          provider: defaultProviderType,
          supportsSubscription: defaultProvider?.supportsSubscription(subscription),
          isHealthy: defaultProvider ? this.isProviderHealthy(defaultProvider) : false
    ...
    if (!excludeList.includes(fallbackProviderType)) {
      ...
      if (fallbackProvider?.supportsSubscription(subscription)) {
        ...
        this.logger.warn({
          provider: fallbackProviderType,
          supportsSubscription: fallbackProvider?.supportsSubscription(subscription)
    ...
    for (const [providerType, provider] of this.providers) {
      if (!excludeList.includes(providerType) && provider.supportsSubscription(subscription)) {
        this.logger.warn({ 
   ...
   */
  async validateSubscription(subscription: ExtendedStreamType): Promise<ValidationResult> {
   ...
   */
  getCapableProviders(subscription: ExtendedStreamType): TranscriptionProvider[] {
    return Array.from(this.providers.values())
      .filter(provider => provider.supportsSubscription(subscription));
  }
  
  /**
   * Get healthy providers for a subscription
   */
  getHealthyProviders(subscription: ExtendedStreamType): TranscriptionProvider[] {
    return this.getCapableProviders(subscription)
  ...
  private getSuggestedAlternatives(subscription: ExtendedStreamType): string[] {
    ...
    allCapabilities.forEach(cap => {
      cap.transcriptionLanguages.forEach(lang => supportedTranscriptionLanguages.add(lang));
    });
