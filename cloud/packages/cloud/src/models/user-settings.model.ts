...
UserSettingsSchema.index({ userId: 1, lastUpdated: -1 });
...
UserSettingsSchema.methods.updateSettings = async function(
  ...
  logger.info(`Settings updated for user ${this.userId}`, {
    userId: this.userId.toString(),
    updatedKeys: Object.keys(newSettings),
  });
};

UserSettingsSchema.methods.getSettings = function(this: UserSettingsI): Record<string, any> {
  return Object.fromEntries(this.settings.entries());
};

UserSettingsSchema.methods.getSetting = function(this: UserSettingsI, key: string): any {
