...
// Automatic one-time migration/backfill to ensure userId/email consistency
// and avoid userId:null duplicates that violate legacy unique index on userId.
(async function migrateUserSettings() {
  const run = async () => {
    try {
      // Backfill email from userId when email is missing/empty and userId exists
      await UserSettings.updateMany(
        ...
        [
          {
            $set: {
              email: { $toLower: { $trim: { input: "$userId" } } },
            },
          },
        ],
      );

      // Backfill userId from email when userId is missing/empty and email exists
      await UserSettings.updateMany(
        {
          $and: [
            {
        ...
        [
          {
            $set: {
              userId: { $toLower: { $trim: { input: "$email" } } },
            },
          },
        ],
      );

      // Normalize casing/whitespace for both fields where present
      await UserSettings.updateMany(
        {
          $or: [
            { email: { $type: "string" } },
      ...
      // Ensure an index on email exists. Try unique first; if it fails due to duplicates, fall back to non-unique.
      try {
        await UserSettings.collection.createIndex(
          { email: 1 },
          { unique: true },
      ...
      // Keep legacy userId unique index intact for backward compatibility.
      logger.info("UserSettings migration completed");
    } catch (err) {
      logger.error({ err }, "UserSettings migration error");
    }
