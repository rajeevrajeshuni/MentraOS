...
export const validateAppApiKey = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    logger.warn('App API Key Middleware: Missing or invalid Authorization header');
  ...
  try {
    ...
    if (!isValid) {
      logger.warn(`App API Key Middleware: Invalid API Key for package ${app.packageName}`);
    ...
    logger.error('App API Key Middleware Error:', error);
...
export const hashWithApiKey = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (packageName) {
      ...
      // For now, we'll assume packageName is available somehow.
      // A more robust approach might involve looking up the API key hash directly.
      logger.warn('App API Key Middleware: Package name not provided for API key hashing.');
    ...
    // Attach the validated app object to the request
    (req as any).app = app;
    logger.info(`App API Key Middleware: Generated hash for ${app.packageName}`);
    next();
