...
router.post('/set-datetime', (req, res) => {
  ...
  if (!coreToken || !datetime || isNaN(Date.parse(datetime))) {
    return res.status(400).json({ error: 'Missing or invalid coreToken or datetime (must be ISO string)' });
  }
  ...
  try {
    ...
    if (!userId) {
      return res.status(401).json({ error: 'Invalid core token - missing user email' });
    }
