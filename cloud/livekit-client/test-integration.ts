...
const LIVEKIT_URL =
  process.env.LIVEKIT_URL || "wss://augmentos-playground.livekit.cloud";
const TEST_ROOM = "test-go-bridge-room";
const TEST_USER = "test-user-123";

// Test audio file (16kHz PCM)
const TEST_AUDIO_FILE = path.join(
  __dirname,
  "..",
  "cloud-client",
  "audio",
  "test-audio-16khz.pcm",
);
...
async function runTest() {
  ...
  try {
    ...
    console.log(
      `  Audio Receive: ${audioReceived ? "✅" : "⚠️ (No audio received - normal if no other publisher)"}`,
    );
