...
async function run() {
  ...
  const { Room, AudioStream, RoomEvent, TrackKind, AudioSource, LocalAudioTrack, TrackSource } = require('@livekit/rtc-node');
  const subRoom = new Room();
  await subRoom.connect(url, subToken, { autoSubscribe: true, timeout: 15000 });
  console.log('🎧 Subscriber connected');

  let receivedBuffers: Buffer[] = [];
  let recvSamples = 0;
  const subReady = new Promise<void>((resolve) => {
    subRoom.on(RoomEvent.TrackSubscribed, async (track: any, _pub: any, participant: any) => {
      if (track.kind !== TrackKind.KIND_AUDIO) return;
      console.log('🎙️  Subscribed to audio from', participant.identity);
      const stream = new AudioStream(track);
      for await (const frame of stream) {
        const sr = (frame as any).sampleRate ?? 48000;
        const ch = (frame as any).channels ?? (frame as any).channelCount ?? 1;
        ...
        if (dataAny instanceof Int16Array) {
          ...
          const view = new Int16Array((dataAny?.buffer as ArrayBuffer) || frame.data.buffer);
          monoInt16 = view;
        }
        const b = Buffer.from(
          (monoInt16.buffer as ArrayBuffer).slice(monoInt16.byteOffset, monoInt16.byteOffset + monoInt16.byteLength),
        );
        receivedBuffers.push(b);
        recvSamples += monoInt16.length;
        // Heuristic: if we received as many samples as the source, resolve
  ...
  // Wait subscriber to finish
  await Promise.race([subReady, delay(3000)]);

  // Write subscriber output WAV
