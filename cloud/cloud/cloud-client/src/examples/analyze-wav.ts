...
function readWav(p: string) {
  ...
  return { buf, audioFormat, channels, sampleRate, bitsPerSample, pcm };
}
...
function analyzePcm16Mono(pcm: Buffer, sampleRate: number) {
  ...
  // Frequency estimate via zero-crossings (positive-going)
  ...
  if (crossings.length > 1) {
    ...
    for (let i = 1; i < crossings.length; i++) {
      ...
      if (period > 0) { sumPeriod += period; count++; }
    }
    const meanPeriod = sumPeriod / Math.max(1, count);
    if (meanPeriod > 0) freq = sampleRate / meanPeriod;
  }
  // Rough spectral cleanliness: proportion of large inter-sample jumps (possible click/static)
  let bigJumps = 0;
  for (let i = 1; i < samples.length; i++) {
...
async function main() {
  ...
  const { audioFormat, channels, sampleRate, bitsPerSample, pcm } = readWav(wavPath);
