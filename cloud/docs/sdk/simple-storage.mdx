...
```typescript
await session.simpleStorage.removeItem('temporaryData');
```
...
```typescript
// Legacy methods (use get/set instead)
await session.simpleStorage.getItem('key');    // Use: get('key')
await session.simpleStorage.setItem('key', 'value'); // Use: set('key', 'value')
```
...
| `removeItem(key)` | `Promise<boolean>` | Delete key |
