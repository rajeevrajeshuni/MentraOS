...
{
    int32_t    ch         = 0;
    size_t size       = ((sizeof(LC3_Enc) + 3) & ~3);
    ...
    for (ch = 0; ch < channels; ch++)
    {
        ...
        size += ((sizeof(EncSetup) + 3) & ~3);
        size += ((4 * DYN_MAX_MDCT_LEN(samplerate) + 3) & ~3);
...
LC3_Error FillEncSetup(LC3_Enc *encoder, int32_t samplerate, int32_t channels)
{
    ...
    encoder->r12k8_mem_in_len  = extract_l(L_shr_pos(Mpy_32_16_asm(encoder->fs, 20972), 9));
...
void set_enc_frame_params(LC3_Enc *encoder)
{
    ...
    encoder->frame_length = extract_l(L_shr_pos(Mpy_32_16_asm(encoder->fs, 20972), 6)); /* fs * 0.01*2^6 */
...
LC3_Error update_enc_bitrate(LC3_Enc *encoder, int32_t bitrate)
{
    ...
    if (encoder->combined_channel_coding)
    {
        totalBytes = fec_get_data_size(encoder->epmode, encoder->combined_channel_coding,
                                          bitrate * (int32_t)encoder->frame_length / (8 * encoder->fs_in));

        encoder->channel_setup[0]->n_pccw =
            fec_get_n_pccw(bitrate * (int32_t)encoder->frame_length / (8 * encoder->fs_in), encoder->epmode,
    ...
    {
        totalBytes = bitrate * (int32_t)encoder->frame_length / (8 * encoder->fs_in);
    }
