...
int32_t get_decoder_mem_size(int32_t samplerate, int32_t channels, LC3_PlcMode plc_mode)
{
    int32_t    ch         = 0;    
    size_t size       =  ((sizeof(LC3_Dec) + 3) & ~3);
    int32_t    max_len = DYN_MAX_LEN_EXT(samplerate); /*NB  sing 80 as minimum value changes BE for NB */

    for (ch = 0; ch < channels; ch++)
    {
        //DecSetup *setup = balloc(decoder, &size, sizeof(DecSetup));
        size += ((sizeof(DecSetup) + 3) & ~3);
        int32_t q_old_len = MIN(samplerate/100, MAX_BW);  /* default init to 10ms frame size q_old length */
        int32_t x_old_len = DYN_MAX_LEN_PCM_PLC(samplerate) ; /* max(pitchmax + frame ms,  M+1 + pitchmax + frame/2) */
        ...
        #else
        size += ((2 * (max_len + max_len / 40 - 2) + 3) & ~3);
        size += ((2 * (max_len + CEILING(MAX_PITCH_12K8 * max_len, 128)+ (max_len / 80)) + 3) & ~3);
        size += ((2 * DYN_MAX_MDCT_LEN(samplerate) + 3) & ~3);
        size += ((2 * q_old_len + 3) & ~3);
        #endif
        ...
        if (plc_mode != LC3_PLC_STANDARD)
        {
            int32_t max_plocs = DYN_MAX_PLOCS(samplerate);
            #if 0
...
int32_t alloc_decoder(LC3_Dec *decoder, int32_t samplerate, int32_t channels, LC3_PlcMode plc_mode)
{
    ...
    for (ch = 0; ch < channels; ch++)
    {
        ...
        int32_t q_old_len = MIN(samplerate/100, MAX_BW);  /* default init to 10ms frame size q_old length */
        int32_t x_old_len = DYN_MAX_LEN_PCM_PLC(samplerate) ; /* max(pitchmax + frame ms,  M+1 + pitchmax + frame/2) */
        int32_t fs_idx    = (samplerate/10000);   /* floor= integer truncation is needed  here */ 
...
LC3_Error FillDecSetup(LC3_Dec *decoder, int32_t samplerate, int32_t channels, LC3_PlcMode plc_mode)
{
    ...
    decoder->ltpf_mem_x_len = extract_l(L_shr_pos(Mpy_32_16_asm(max(16000, decoder->fs), 16778), 11)) - 2;
    decoder->ltpf_mem_y_len = extract_l(L_shr_pos(Mpy_32_16_asm(decoder->fs, 18678) - 1, 5)) + 1 +
                              extract_l(L_shr_pos(Mpy_32_16_asm(max(16000, decoder->fs), 16778), 12));
    ...
    for (ch = 0; ch < decoder->channels; ch++)
    {
       ...
       if (decoder->plcMeth != LC3_PLC_STANDARD)
       {
...
#if ( PHECU_XFP_LA == 0 )
          int16_t oneMsTab_LA[5] = { 0, 0, 0, 0, 0 };
#else
#if ( PHECU_XFP_LA == 4 )
          int16_t oneMsTab_LA[7] = { 0 /*unused*/, 2, 4, 6, 8, 10 /*unused*/, 12 };
#else
          int16_t oneMsTab_LA[7] = { 0 /*unused*/, 8, 16, 24, 32, 40 /*unused*/, 48 };
#endif
#endif
          int16_t oneMsTab[5] = { 8, 16, 24, 32, 48 };
          ...
          setup->plcAd->PhECU_fs_idx_fx = mult(decoder->frame_length, (int16_t)(32768.0 / 99.0)); /* truncation needed , i.e no rounding can be applied here */
          /* idx=frame/80,  0=8kHZ, 1=16kHz, 2=24 kHz, 3=32 kHz 5=*, 4=48 */

          setup->plcAd->PhECU_frame_ms = (int16_t)(decoder->frame_dms*0.1);  /* needed in PLCUpdate and PLC main functions, adjusted in first frame */
...
{
   decoder->frame_length = extract_l(L_shr_pos(Mpy_32_16_asm(decoder->fs, 20972), 6)); /* fs * 0.01*2^6 */
   ...
   switch (decoder->frame_dms)
   {
    case 25:
       ...
       decoder->yLen = min(MAX_BW >> 2, decoder->frame_length);
    ...
    case 50:
       ...
       decoder->yLen = min(MAX_BW >> 1, decoder->frame_length);
...

   {
      ...
      for (ch = 0; ch < decoder->channels; ch++)
      {
         ...
         if (setup->plcAd != NULL) {    /*only set  if plcAd was actually allocated */
           setup->plcAd->PhECU_frame_ms = (int16_t)(decoder->frame_dms*0.1);  /* needed in processPLCupdate(),  now set properly set in first frame /second time  */
          }
      }
   }
...
{
    int32_t tmp = 0, totalBits = 0;
    int32_t channel_bytes = 0;
