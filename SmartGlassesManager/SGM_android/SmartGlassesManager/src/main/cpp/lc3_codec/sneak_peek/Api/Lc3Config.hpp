...
class Lc3Config 
{
    public:
         ...
         *
         */
        //Lc3Config(uint32_t Fs_, FrameDuration N_ms_, uint8_t Nc_);
        bool Inititalize(uint32_t Fs_, FrameDuration N_ms_, uint8_t Nc_, bool interlace = false);
         ...
         */
        float getNmsValue() const;

        void SetInterlace(bool) const;
    private:
        ...
        uint16_t getZ(uint16_t NF, FrameDuration N_ms);
        uint8_t getNB(uint8_t Fs_ind, FrameDuration N_ms);
    ...
    public:
        ...
        uint32_t Fs;        // Sampling rate (in Hz)
        uint8_t Fs_ind;     // Sampling rate index (see also Section 3.2.2 "Sampling rates" (dr09r07)
        FrameDuration N_ms; // Frame duration -> see Lc3Config constructor documentation
                            // Note: that the actual frame duration is longer by a factor of 480/441
                            //       if the sampling rate is 44100 Hz
        uint16_t NF;        // Number of samples processed in one frame of one channel (also known as frame size)
        uint16_t NE;        // Number of encoded spectral lines (per frame and channel)
        uint16_t Z;         // Number of leading zeros in MDCT window
        uint8_t Nc;         // Number of channels
        uint8_t N_b;        // Number of bands

		bool isInterlace = false;
    public:
        virtual void* Alloc(int size)const;
        virtual void Free(void*& ptr)const;
        virtual int GetMemUsed() const;
        virtual int GetMemUsedMax() const;
    public:
        //virtual void addDatapoint(const char* label, const void* pData, uint16_t sizeInBytes)const;
        //virtual void log(const char* label, const void* pData, uint16_t sizeInBytes)const;
        virtual int print(int ch, const char* str, ...)const;
    public:
        void* _runtime_temp32 = 0;//NF*4(bytes)
        void* _runtime[16];
        template<class T>
        T& Runtime() const {
            return *(T*)_runtime;
