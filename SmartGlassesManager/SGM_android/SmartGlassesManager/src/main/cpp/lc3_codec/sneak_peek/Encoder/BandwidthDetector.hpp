...
class BandwidthDetector :public Lc3Base
{
    public:
        BandwidthDetector(const Lc3Config& cfg);
        ~BandwidthDetector();

        void registerDatapoints();

        void run(const float* const E_B);  // Note: fs-dependent N_b not needed since processing occurs only for fs>8000
