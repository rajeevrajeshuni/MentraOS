...
public:
     ...
     */
    //Lc3Decoder(const Lc3Config& cfg, uint8_t bits_per_audio_sample_dec_ = 16, uint16_t byte_count_max_dec_ = 400);
    bool Initialize(const Lc3Config& cfg, uint8_t bits_depth = 16, uint8_t bits_align = 0, uint16_t byte_count_max_dec_ = 400);
    void Uninitialize();
    ...
    uint8_t _bits_depth = 0;
    uint8_t _bits_align = 0;
    uint16_t byte_count_max_dec = 0;

    // encoding error (see return values of "run" methods)
    enum : uint8_t {
        ERROR_FREE = 0x00,
        INVALID_CONFIGURATION = 0x01,
        INVALID_BYTE_COUNT = 0x02,
        INVALID_X_OUT_SIZE = 0x03,
        INVALID_BITS_PER_AUDIO_SAMPLE = 0x04,
        DECODER_ALLOCATION_ERROR = 0x05,
    };
     ...
     *                      the error INVALID_X_OUT_SIZE when x_out_size != _cfg.NF.
     *
     ...
     *                Note: this parameter has been introduced for clarity and
     *                      verification purpose only. The method will return
     *                      the error INVALID_X_OUT_SIZE when x_out_size != _cfg.NF * lc3Conig.Nc.
     *
    ...
    ///new
    //uint8_t run(const uint8_t* bytes, uint16_t nbytes, uint8_t BFIs, int16_t** x_out, uint8_t& BECs_detect);
    uint8_t run(const uint8_t *bytes, uint16_t nbytes, uint8_t BFIs, void **x_out, uint8_t &BECs_detect);
    uint8_t run_interlaced(const uint8_t *bytes, uint16_t nbytes, uint8_t BFIs, void *x_out, uint8_t &BECs_detect);
private:
    dec::DecoderFrame* decoderList[2];    
    void *_outbuff = 0;
};
}
